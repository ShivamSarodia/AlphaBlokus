...
The strongest AlphaBlokus model, running with 2000 rollouts, achieves a win rate of TODO% against Pentobi head-to-head at level 9, the maximum Pentobi level. Importantly, I have not attempted to rigorously compare win rates with equal compute. Pentobi is primarily limited by CPU while AlphaBlokus is bottlenecked on GPU, so direct comparison is not trivial; however, even with AlphaBlokus running its neural network evaluation on CPU on my local machine, AlphaBlokus plays faster and achives a win-rate >50% of Pentobi at level 9.
