...
def test_evaluate_batch():
    ...
    assert values.shape == (batch_size, 4), (
        f"Expected values shape ({batch_size}, 4), got {values.shape}"
    )
    ...
    assert policy_logits.shape == expected_policy_shape, (
        f"Expected policy shape {expected_policy_shape}, got {policy_logits.shape}"
    )
    ...
    np.testing.assert_allclose(
        ...
        1.0,
        rtol=1e-6,
        err_msg="Values should be properly softmaxed (sum to 1)",
    )
