...
export async function evaluate(
  ...
  const outputs = await session.run({
    board: new ort.Tensor('float32', Float32Array.from(board), [1, 4, BOARD_SIZE, BOARD_SIZE]),
    piece_availability: new ort.Tensor('float32', Float32Array.from(pieceAvailability), [1, 4, NUM_PIECES]),
  })
