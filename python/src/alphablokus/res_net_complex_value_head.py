...
class ValueHead(nn.Module):
    def __init__(self, net_config: NetworkConfig, game_config: GameConfig):
        ...
        self.layers = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(
                in_channels=net_config.value_head_channels,
                out_channels=net_config.value_head_channels,
                kernel_size=3,
                stride=1,
                padding=1,
    ...
    def forward(self, x):
        ...
        coords = torch.linspace(0.0, 1.0, board_size, device=x.device, dtype=x.dtype)
        yy, xx = torch.meshgrid(coords, coords, indexing="ij")
        pos = torch.stack([xx, yy], dim=0).unsqueeze(0).expand(x.shape[0], -1, -1, -1)
