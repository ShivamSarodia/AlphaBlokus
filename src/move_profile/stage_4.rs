...
use crate::move_profile::stage_3::Stage3MoveProfile;
...
pub fn compute_stage_4_move_profiles(
    ...
    let stage_4_move_profiles = stage_3_move_profiles
        ...
        .map(|stage_3_move| {
            let mut moves_ruled_out_for_player = Vec::new();
            let mut moves_ruled_out_for_all = Vec::new();
            let mut moves_enabled_for_player = Vec::new();

            for other_stage_3_move in &stage_3_move_profiles {
                let mut ruled_out_for_player = false;
                let mut ruled_out_for_all = false;
                let mut enabled_for_player = false;

                // If the other move occupies any of this move's edge cells, that other move is
                // ruled out for both the player and all other players.
                if other_stage_3_move
                    .occupied_cells
                    .overlaps(&stage_3_move.edge_cells)
                {
                    ruled_out_for_player = true;
                    ruled_out_for_all = true;
                }

                // If the other move uses the same piece as this move, that other move is ruled
                // out for the player but not other players.
                ...
                // not ruled out.
                if !ruled_out_for_all
                    && !ruled_out_for_player
                    && other_stage_3_move
                        .occupied_cells
                        .overlaps(&stage_3_move.corner_cells)
                {
                    enabled_for_player = true;
                }

                if ruled_out_for_player {
                    moves_ruled_out_for_player.push(other_stage_3_move.index);
                }
                if ruled_out_for_all {
                    moves_ruled_out_for_all.push(other_stage_3_move.index);
                }
                if enabled_for_player {
                    moves_enabled_for_player.push(other_stage_3_move.index);
                }
            }
            ...
            Stage4MoveProfile {
                ...
                moves_ruled_out_for_player,
                moves_ruled_out_for_all,
                moves_enabled_for_player,
            }
        })
