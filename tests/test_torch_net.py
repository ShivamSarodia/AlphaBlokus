...
def test_policy_flatten_mapping_correctness(
    policy_flatten, test_config, moves_data_dict
):
    ...
    for move_idx in range(test_config.game.num_moves):
        ...
        assert (
            expected_occupieds[expected_x, expected_y]
            or (
                expected_x < board_size - 1
                and expected_occupieds[expected_x + 1, expected_y]
            )
            or (expected_x > 0 and expected_occupieds[expected_x - 1, expected_y])
            or (
                expected_y < board_size - 1
                and expected_occupieds[expected_x, expected_y + 1]
            )
            or (expected_y > 0 and expected_occupieds[expected_x, expected_y - 1])
        )

        for batch_idx in range(batch_size):
            # What we expect to find in the output
            expected_value = input_tensor[
                batch_idx, expected_po, expected_x, expected_y
            ]
...
def test_policy_flatten_with_gradient_flow(policy_flatten, test_config):
    ...
    input_tensor = torch.randn(
        batch_size, num_piece_orientations, board_size, board_size, requires_grad=True
    )
    ...
    assert input_tensor.grad is not None, (
        "Gradients should flow back through PolicyFlatten"
    )
    ...
    assert (
        torch.count_nonzero(input_tensor.grad)
        == batch_size * test_config.game.num_moves
    )
